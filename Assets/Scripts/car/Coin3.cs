...
public class Coin3 : MonoBehaviour {
	...
	void Start(){
		Text_3.guiText.material.color = Color.black;
	...
	void Update() 
	{
		if (m_state == eInteractiveState.Active) {
			...
			if (Seconds <= 0) {
				...
				if(connect.c==1){
					Text_3.text = PlayerPrefs.GetString("Player1Name");
				}
				if(connect.c==2){
					Text_3.text = PlayerPrefs.GetString("Player2Name");
				}
				if(connect.c==3){
					Text_3.text = PlayerPrefs.GetString("Player3Name");
				}
